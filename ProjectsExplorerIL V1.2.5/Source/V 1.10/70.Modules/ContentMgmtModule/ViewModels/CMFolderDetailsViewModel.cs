...
namespace ContentMgmtModule
{
    public class CMFolderDetailsViewModel : ViewModelBase
    {
        ...
        [StringLength(20, ErrorMessage = "Maximum length (20 characters) exceeded.")]
        public string DefaultVNPrefix
        {
            get
            {
                if (CMFolder != null)
                {
                    ...
                    return CMFolder.DefaultVNPrefix;
                }
                else
                {
                    return string.Empty;
            ...
            set
            {
                if (Node != null)
                {
                    CMFolder.DefaultVNPrefix = value;
                    RaisePropertyChanged("DefaultVNPrefix");
                    MessageMediator.NotifyColleagues(this.WorkspaceId + "OnIsDirtyNodeReceived", this.Node);
                }
            }
        }

        [Range(1.0, 99, ErrorMessage = "Please enter number between 1 and 99")]
        [StringLength(4, ErrorMessage = "Maximum length exceeded (4)")]
        public string DefaultVNStartValue
        {
            get
            {
                if (CMFolder != null)
                {
                    ...
                    return CMFolder.DefaultVNStartValue;
                }
                else
                {
                    return string.Empty;
            ...
            set
            {
                if (Node != null)
                {
                    CMFolder.DefaultVNStartValue = value;
                    RaisePropertyChanged("DefaultVNStartValue");
                    RaisePropertyChanged("IsIncrementEnabled");
                    RaisePropertyChanged("IsToolTipEnabled");
                    RaisePropertyChanged("DefaultVNIncrement");
                    MessageMediator.NotifyColleagues(this.WorkspaceId + "OnIsDirtyNodeReceived", this.Node);
                }
            }
        }

        private bool _IsIncrementEnabled;
        public bool IsIncrementEnabled
        {
            get
            {
                if (UpdateModeProperty &&
                 DefaultVNStartValue != null
                && !string.IsNullOrWhiteSpace(DefaultVNStartValue)
                && !string.IsNullOrEmpty(DefaultVNStartValue))
                {
                    return true;
                }
                else
                {
                    if (UpdateModeProperty)
                    {
                        DefaultVNIncrement = string.Empty;
                        RaisePropertyChanged("DefaultVNIncrement");
                    }
                    return false;
                }
        ...
        [Required(ErrorMessage = "The field is mandatory when Start Value is populalted")]
        [Range(0, 1.0, ErrorMessage = "Please enter number between 0 and 1")]
        [StringLength(4, ErrorMessage = "Maximum precision value is 2")]
        public string DefaultVNIncrement
        {
            get
            {
                if (CMFolder != null )
                {
                    if (!(string.IsNullOrEmpty(CMFolder.DefaultVNIncrement) || string.IsNullOrWhiteSpace(CMFolder.DefaultVNIncrement)))
                    {
                        return CMFolder.DefaultVNIncrement;
                    }
                    if ((string.IsNullOrEmpty(CMFolder.DefaultVNIncrement) || string.IsNullOrWhiteSpace(CMFolder.DefaultVNIncrement)) 
                        && !((string.IsNullOrEmpty(CMFolder.DefaultVNStartValue) || string.IsNullOrWhiteSpace(CMFolder.DefaultVNStartValue))))
                    {
                        return CMFolder.DefaultVNIncrement;
                    }
                    if ((string.IsNullOrEmpty(CMFolder.DefaultVNIncrement) || string.IsNullOrWhiteSpace(CMFolder.DefaultVNIncrement)) &&
            ...
            set
            {
                if (Node != null)
                {
                    ...
                    {
                        CMFolder.DefaultVNIncrement = value;
        ...
        public CMFolderDetailsViewModel(Guid workspaceId, CMTreeViewNodeViewModelBase TV)
        {
            ...
            CMFolder.DefaultVNPrefix = initCmFolder.DefaultVNPrefix;
            CMFolder.DefaultVNStartValue = initCmFolder.DefaultVNStartValue;
            CMFolder.DefaultVNIncrement = initCmFolder.DefaultVNIncrement;
        ...
        #region Commands
            ...
            private bool CanExecuteSaveCommand()
            {
                if ((String.IsNullOrEmpty(FolderName) || FolderName.Length > 50 || FolderDescription.Length > 1000 || !((CMTreeViewFolderNodeViewModel)(Node)).TreeNode.ExistPermission("Update")))
                    return false;
                RangeAttribute ra = new RangeAttribute(1.0, 99);
                if (!ra.IsValid(DefaultVNStartValue))
                    return false;

                RangeAttribute raIncr = new RangeAttribute(0, 1.0);
                if (!raIncr.IsValid(DefaultVNIncrement))
                {
                    return false;
                }

                StringLengthAttribute sla = new StringLengthAttribute(4);
                if (!sla.IsValid(DefaultVNStartValue))
                    return false;

                StringLengthAttribute slaIncr = new StringLengthAttribute(4);
                if (!sla.IsValid(DefaultVNIncrement))
                {
                    return false;
                }

                RequiredAttribute rqa = new RequiredAttribute();
                if (!rqa.IsValid(DefaultVNIncrement))
                {
                    return false;
                }
                return true;
