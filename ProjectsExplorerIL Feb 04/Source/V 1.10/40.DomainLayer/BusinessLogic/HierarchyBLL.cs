...
namespace ATSBusinessLogic
{
    public class HierarchyBLL
    {
        ...
        public static DataTable GetHierarchyDataTable()
        {
            try
            {
                StringBuilder QryStr = new System.Text.StringBuilder();
                QryStr.Append("Select * FROM PE_Hierarchy ");
                // Fetch the DataTable from the database
                DataTable HierarchyTable = Domain.PersistenceLayer.FetchDataTable(QryStr.ToString(), CommandType.Text, null);
                // Populate the collection
                ...
                {
                    String logMessage = "Failed to retrieve PE_Hierarchy data table.";
                    Domain.SaveGeneralErrorLog(logMessage);
                    return null;
                }
            }
        ...
        public static HierarchyModel GetHierarchyRowFromDataTable(long Id, DataTable Hierarchy)
        {
            // Initialize work fields
            HierarchyModel Node = Domain.GetBusinessObject<HierarchyModel>();
            try
            {
                ...
                DataRow[] ResTable = Hierarchy.Select(selectCondition);
                // Populate the collection
                if (ResTable != null && ResTable.Count() == 1)
                {
                    DataRow DataRow = ResTable[0];
                    //
                    Node.Id = (int)DataRow["Id"];
                    Node.IsNew = false;
                    Node.IsDirty = false;
