...
{
    public class TreeViewVersionNodeViewModel : TreeViewNodeViewModelBase
    {
        public TreeViewVersionNodeViewModel(Guid workspaceId, HierarchyModel Hierarchy)
        ...
        public TreeViewVersionNodeViewModel(Guid workspaceId, HierarchyModel Hierarchy, TreeViewNodeViewModelBase ParentNode)
            : base(workspaceId, Hierarchy, ParentNode)
        {
            ...
            {
                activeVersion = ATSBusinessLogic.VersionBLL.GetActiveVersion(this.Hierarchy.Id);
            }
            else
                activeVersion = ATSBusinessLogic.VersionBLL.GetActiveVersion(this.Hierarchy.GroupId);
        ...
        {
            MessageMediator.NotifyColleagues(WorkSpaceId + "ShowVersionDetails", this); //Will be returned to the Explorer Main signed for this message
        }
       ...
       private static VersionModel activeVersion;
        ...
        private bool validate()
        {
            try
            {
                if (showMessage() == DialogResults.OK)
                {
                    ...
                    if (OpenResult)
                    {
                        MessageMediator.NotifyColleagues(this.WorkSpaceId + "OnUpdateVersionReceived", this);
                        if (Hierarchy.GroupId == -1)
                        {
                            activeVersion = ATSBusinessLogic.VersionBLL.GetActiveVersion(this.Hierarchy.Id);
                        }
        ...
        private void ExecuteRapidExecutionVersionCommand()
        {
            try
            {
                MessageMediator.NotifyColleagues(WorkSpaceId + "OnRapidExecutionVersionStartReceived", this);
        ...
        private void ContentExecution()
        {
            ...
            try
            {
                ...
                versionHierarchy.VM.Contents = (ObservableCollection<ContentModel>)activeVersionCMs;
            ...
            t.Start();
                ...
                if (Status.errorId != string.Empty)
                {
                    ProjectsExplorerViewModel.ShowErrorAndInfoMessage(Convert.ToInt32(Status.errorId), Status.errorParams);
                    ProgressText = "0 %";
                    MessageMediator.NotifyColleagues(WorkSpaceId + "OnVersionProgressTextReceived", this);
                    return;
                }
                #endregion
        ...
        {
            MessageMediator.NotifyColleagues(WorkSpaceId + "OnClearVersionViewModelReceived", this);
        }
        ...
        {
            MessageMediator.NotifyColleagues(WorkSpaceId + "OnRapidExecutionVersionReceived", this);
        }
        #endregion
