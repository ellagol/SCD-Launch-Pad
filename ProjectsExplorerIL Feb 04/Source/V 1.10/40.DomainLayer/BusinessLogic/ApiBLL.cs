    ...
    public  class ApiBLL
    {
            ...
            public static Dictionary<int, HierarchyModel> GetProjectsByContentVersionID(int contentVersionID)
        {
            ...
            try
            {
                ...
                //Improving performance - selecting Full Path recursively
                SB.Append(" with cte (NodeId,NodeName,FullPath) as  ");
                SB.Append(" (select id,cast(name as varchar(1000)), cast(name as varchar(1000))  ");
                SB.Append(" from PE_Hierarchy where ParentId is null  ");
                SB.Append(" union all  ");
                SB.Append(" select n.Id, cast(n.Name as varchar(1000)), CONVERT(varchar(1000),cte.FullPath + '/' + n.Name) as name  ");
                SB.Append(" from PE_Hierarchy n join cte on n.ParentId = cte.NodeId )  ");
                SB.Append(" SELECT DISTINCT  ");
                SB.Append(" (CASE H1.NodeType WHEN 'P' THEN H1.Id WHEN 'T' THEN H1.Id WHEN 'G' THEN H2.Id END) AS Id, FullPath  ");
                SB.Append(" FROM PE_Hierarchy AS H1  ");
                SB.Append(" join cte on ID = NodeId ");
                SB.Append(" INNER JOIN PE_Version AS V  ");
                SB.Append(" ON H1.Id = V.HierarchyId   ");
                SB.Append(" LEFT OUTER JOIN PE_Hierarchy AS H2 ON H1.Id = H2.GroupId  ");
                ...
                if (ResTable != null)
                {
                    DataTable Hierarchy = HierarchyBLL.GetHierarchyDataTable();
                    DataTable Version = VersionBLL.GetVersionsDataTable();

                    Hierarchy.PrimaryKey = new DataColumn[] { Hierarchy.Columns["Id"] };
                    //foreach (DataRow dr in ResTable.Rows)
                    for (int i = ResTable.Rows.Count - 1; i >= 0; i--)
                    {
                        DataRow dr = ResTable.Rows[i];
                        long HierarchyID = Convert.ToInt32(dr["Id"]);
                        string selectCondition = "Id = " + HierarchyID;
                        DataRow[] hierarchyDataRowArray = Hierarchy.Select(selectCondition);
                        //HierarchyModel HM = HierarchyBLL.GetHierarchyRow(HierarchyID);
                        HierarchyModel HM = HierarchyBLL.GetNodeModel(hierarchyDataRowArray[0]);
                        //Performance
                        HM.VM = VersionBLL.GetActiveVersionFromDataTable(HierarchyID, Version);
                        HM.GetAllVersions = VersionBLL.GetAllVersionsFromDataTable(HierarchyID, Version);

                        //HM.VM = VersionBLL.GetActiveVersion(HierarchyID);
                        //HM.GetAllVersions = VersionBLL.GetAllVersions(HierarchyID);
                        //Performance
                        ...
                        HM.TreeHeader = Convert.ToString(dr["FullPath"]);
